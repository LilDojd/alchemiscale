...
from .cypher import cypher_join, cypher_escape, cypher_repr, CypherExpression
...
def unwind_create_relationships_query(
    data, rel_type, start_node_key=None, end_node_key=None, keys=None
):
    """Generate a parameterised ``UNWIND...CREATE`` query for bulk
...
class NodeKey(object):
    ...
    def key_value_string(self, value, ix):
        return ", ".join(
            "%s:%s[%s]" % (cypher_escape(key), value, cypher_repr(ix[i]))
...
def _merge_clause(name, merge_key, value, keys, prefix="(", suffix=")"):
    ...
    merge_keys = nk.keys()
    if len(merge_keys) == 0:
        return "MERGE %s%s%s%s" % (prefix, name, nk.label_string(), suffix)
    elif keys is None:
        return "MERGE %s%s%s {%s}%s" % (
            prefix,
            name,
            nk.label_string(),
            nk.key_value_string(value, merge_keys),
            suffix,
        )
    else:
        return "MERGE %s%s%s {%s}%s" % (
            ...
            name,
            nk.label_string(),
            nk.key_value_string(value, [keys.index(key) for key in merge_keys]),
            suffix,
        )
